...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
      ...
      return h('div', { className: 'hst_card hst_review' }, [
        ...
        h('div', { key: 'grid', className: `hst_reviewGrid${scene === null ? '' : ' hst_reviewGrid2'}` }, [
          ...
          scene === null ? null : h('div', { key: 'frame', className: 'hst_reviewFrame' }, h(DemoFrame, {
            ...
            scene,
            token: `deck-${item.id}`,
            // 抽题图示区：纯画布+步骤条（无标题行、无步骤说明），两栏平分宽度
            // 画布文字舒展；翻面后画布自动缩矮，给左栏答案区让空间
            mode: 'panel',
            minHeight: 280,
            cap: 460,
            compact: true,
     ...
     * @returns {object} React 元素。
     */
    function DemoFrame({ scene, token, mode, onStep, minHeight, cap: capProp, bare: bareProp, ratio: ratioProp, compact: compactProp }) {
