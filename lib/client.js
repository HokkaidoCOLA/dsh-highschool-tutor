 ...
 *        今日   倒计时、今日待复习、复习闯关（16:9 翻卡+四档评分）、快速记时长、薄弱点
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      // 复习闯关：固定 16:9 画幅卡片（内容区内部滚动，翻面后评分区钉在底部）
      '.hst_review{display:flex;flex-direction:column;gap:12px}',
      '.hst_reviewStage{width:100%;max-width:calc(60vh * 16 / 9);aspect-ratio:16 / 9;min-height:0;margin-inline:auto;overflow:hidden}',
      '.hst_reviewBody{flex:1 1 auto;min-height:0;overflow-y:auto;display:flex;flex-direction:column;gap:12px;overscroll-behavior:contain;scrollbar-width:thin}',
      '.hst_reviewFoot{flex:none;display:flex;flex-direction:column;gap:10px}',
      ...
      '.hst_q{font-size:15.5px;line-height:1.75;white-space:pre-wrap;word-break:break-word}',
      '.hst_a{font-size:14px;line-height:1.75;white-space:pre-wrap;word-break:break-word;padding:10px 12px;border-radius:10px;background:var(--dsw-alias-bg-module-platform,#2631480a);border-left:3px solid var(--dsw-alias-state-success-primary,#16a34a)}',
      '.hst_expl{font-size:12.5px;line-height:1.7;white-space:pre-wrap;color:var(--dsw-alias-label-secondary,#61666b)}',
      '.hst_grades{display:grid;grid-template-columns:repeat(4,1fr);gap:8px}',
      '.hst_gradeBtn{appearance:none;font:inherit;border-radius:10px;padding:9px 6px;cursor:pointer;display:flex;flex-direction:column;align-items:center;gap:2px;border:1px solid var(--dsw-alias-border-l2,#0000001a);background:var(--dsw-alias-bg-layer-3,#fff);color:var(--dsw-alias-label-primary,#0f1115)}',
     ...
     * 卡片固定 16:9 画幅（高随宽自适应），题干/答案在内容区内部滚动，
     * 翻面后四档评分钉在卡片底部；键盘：空格/回车翻面，1-4 评分，Esc 退出。
    ...
    function ReviewRunner({ subject, compact, onExit }) {
      ...
      if (error !== null) {
        return h('div', { className: 'hst_card hst_review hst_reviewStage' }, [
      ...
      if (queue === null) return h('div', { className: 'hst_card hst_review hst_reviewStage' }, h('div', { className: 'hst_reviewEnd' }, h('p', { className: 'hst_hint' }, '正在取今日队列…')))
    ...
    function DemoFrame({ scene, token, mode, onStep, minHeight }) {
      ...
      if (doc === null) return h('div', { className: 'hst_frameSkeleton', style: { height: `${height}px` } })
      return h('iframe', {
        ref,
        className: 'hst_frame',
