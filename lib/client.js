...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      '.hst_quiz{width:min(940px,96vw);max-height:92vh;overflow:auto;background:var(--dsw-alias-bg-layer-3,#fff);border:1px solid var(--dsw-alias-border-l2,#0000001a);border-radius:16px;box-shadow:0 24px 64px #00000045;padding:14px 16px 18px}',
      ...
      '.hst_reviewGrid2{grid-template-columns:minmax(0,1.2fr) minmax(0,0.8fr)}',
      ...
      return h('div', { className: 'hst_card hst_review' }, [
        ...
        h('div', { key: 'grid', className: `hst_reviewGrid${scene === null ? '' : ' hst_reviewGrid2'}` }, [
          ...
          scene === null ? null : h('div', { key: 'frame', className: 'hst_reviewFrame' }, h(DemoFrame, {
            ...
            scene,
            token: `deck-${item.id}`,
            // 抽题图示区：内容自适应高度（画布+步骤条+步骤说明全部显示），
            // 纯画布模式去掉顶部漂移的标题/副标题文字，画布纵向拉大
            mode: 'panel',
            minHeight: 300,
            cap: 560,
            bare: true,
     ...
     * @returns {object} React 元素。
     */
    function DemoFrame({ scene, token, mode, onStep, minHeight, cap: capProp, bare: bareProp, ratio: ratioProp }) {
