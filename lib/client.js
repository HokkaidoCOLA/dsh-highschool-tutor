...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
        ...
        return h('div', { className: 'hst_card hst_review' }, [
          h('div', { key: 'd', className: 'hst_reviewEnd' }, [
            h('div', { key: 't', className: 'hst_empty' }, done > 0
              ...
              : '今日队列已清空，休息一下 🎉'),
            h('p', { key: 'h', className: 'hst_hint' }, '跟助手说「再来一组」可以换一批；把刚讲完的题也录成卡片，明天就能在这里复习。'),
          ]),
        ])
      }
    ...
    // ── 今日 ────────────────────────────────────────────────────────────────
    /**
     * 今日标签页：倒计时、目标进度、待复习分布、复习入口指引、快速记时长、薄弱点。
    ...
    function TodayTab({ overview, onGoLibrary }) {
      ...
      return h('div', { className: 'hst_body' }, [
        ...
        // 复习入口：已搬到对话里（设置页不再内嵌翻卡界面）
        h('div', { key: 'go', className: 'hst_card' }, [
          h('h3', { key: 't', className: 'hst_h3' }, '怎么开始复习'),
          h('p', { key: 'p', className: 'hst_hint' }, due.total + due.new === 0
            ? '今天没有到期的题。可以让助手把刚讲完的题写成卡片，或去「题库」标签导入内置卡片包。'
            : `复习改在对话里进行：跟我说「抽查我 ${due.total + due.new} 道」或「考考我物理」，我会把今天的卡做成翻卡发到对话里，你在卡上翻面、四档评分，分数直接写入排期。`),
    ...
    /**
     * 会话标题栏徽标：待复习条数 + 高考倒计时，点开看今日分布，复习在对话里发起。
     * @returns {object|null} React 元素。
     */
    function TutorBadge() {
      const overview = useResource('/overview')
      ...
      return h('div', { className: 'hst_popWrap', ref: wrapRef }, [
        ...
        open ? h('div', { key: 'pop', className: 'hst_pop' }, [
          ...
          h('div', { key: 'act', className: 'hst_row' }, [
            h('span', { key: 'h', className: 'hst_hint' }, dueTotal === 0
              ? '今日待复习已清空。在对话里说「抽查我」可以再练一轮。'
              : `在对话里说「抽查我 ${dueTotal} 道」开始复习，翻卡直接发在对话里。`),
          ]),
        ]) : null,
      ])
    }
