 ...
 *      点开看今日分科分布；复习在对话里发起（说「抽查我」即可）。
 *
 * 动态演示与复习翻卡（讲题卡）另注册两处工具视图：
 *   ③ tool.call.toolview（key='tutor_visualize'）→ 对话内嵌讲题卡：
 *      题目 → 16:9 动态演示 → 答案/解析 → 四档评分（写入复习排期），
 *      内容全部来自该次调用的持久化 meta，刷新页面与会话重放都能重新画出来。
 *   ④ tool.call.toolview（key='tutor_review_deck'）→ 对话内嵌复习翻卡套组：
 *      今日到期题一张张翻、当场评分，「重来」的卡本轮末尾重现。
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      // 复习翻卡与讲题卡：内容全量铺开（高度随内容），演示画布固定 16:9
      ...
      '.hst_reviewEnd{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:12px;text-align:center;padding:10px 0 4px}',
      '.hst_reviewFoot{display:flex;flex-direction:column;gap:10px}',
      '.hst_reviewFrame{border-radius:10px;overflow:hidden;background:var(--dsw-alias-bg-module-platform,#2631480a)}',
      ...
      // 小卡片抽题：宽屏居中弹层（设置面板与徽标浮层都窄，挤不下 16:9 的横版卡片）
      '.hst_quizWrap{position:fixed;inset:0;z-index:130;background:#0b0e1473;display:flex;align-items:center;justify-content:center;padding:22px 18px;animation:hst_fade .16s ease}',
      '.hst_quiz{width:min(940px,96vw);max-height:92vh;overflow:auto;background:var(--dsw-alias-bg-layer-3,#fff);border:1px solid var(--dsw-alias-border-l2,#0000001a);border-radius:16px;box-shadow:0 24px 64px #00000045;padding:12px 14px 16px}',
      // 弹层内更紧致：少留白、评分按钮压缩，为长答案腾出同屏空间
      '.hst_quiz .hst_review{gap:10px}',
      '.hst_quiz .hst_reviewFoot{gap:8px}',
      '.hst_quiz .hst_gradeBtn{padding:6px 4px}',
      '.hst_quiz .hst_gradeSub{font-size:10px}',
      '@keyframes hst_fade{from{opacity:0}to{opacity:1}}',
      '@media (prefers-reduced-motion: reduce){.hst_quizWrap{animation:none}}',
      // 抽题双栏：题目与图示并排，一屏放下不用滚；窄屏回退单栏
      '.hst_reviewGrid{display:grid;grid-template-columns:minmax(0,1fr);gap:12px;align-items:start}',
      '.hst_reviewGrid2{grid-template-columns:minmax(0,1fr) minmax(0,1fr)}',
      '.hst_reviewCol{display:flex;flex-direction:column;gap:10px;min-width:0}',
      '@media (max-width:640px){.hst_reviewGrid2{grid-template-columns:minmax(0,1fr)}}',
      '.hst_quiz .hst_a,.hst_quiz .hst_expl{max-width:640px}',
      // 图表
    ...
    /**
     * 四档评分按钮（讲题卡与复习翻卡共用）。
     * @param {{item: object, onGrade: (key: string) => void}} props 题目（需带 preview）。
     * @returns {object} React 元素。
     */
    function GradeButtons({ item, onGrade }) {
      return h('div', { className: 'hst_grades' }, GRADE_BUTTONS.map((g) => h('button', {
        key: g.key,
        type: 'button',
    ...
    /**
     * 答案区（答案 + 解析 + 复习信息行），讲题卡与复习翻卡共用。
     * @param {{item: object}} props 题目。
     * @returns {object} React 元素。
     */
    function AnswerBlock({ item }) {
      return h('div', { key: 'ans', style: { display: 'flex', flexDirection: 'column', gap: 10 } }, [
        ...
        item.srs == null ? null : h('div', { key: 'm', className: 'hst_meta' }, [
          ...
          Number.isFinite(item.masteryScore) ? h('span', { key: 'ms' }, `掌握度 ${item.masteryScore}`) : null,
          item.source === '' ? null : h('span', { key: 'src' }, `来源 ${item.source}`),
        ]),
      ])
    }

    /** 演示场景按题目 id 的轻缓存（同一道题重复出现不重复拉取）。 */
    const demoSceneCache = new Map()

    /**
     * 复习翻卡套组：复习入口已搬到对话里——
     * 题目 →（该题若有演示则显示 16:9 画布）→ 显示答案 → 四档评分 → 下一张。
     * 答「重来」的卡本轮末尾重现；评分直接写进艾宾浩斯排期。
     * 键盘：空格/回车翻面，1-4 评分。
     * @param {{deck: object}} props tutor_review_deck 的持久化 meta。
     * @returns {object} React 元素。
     */
    function DeckCard({ deck, onRestart, onExit }) {
      const [queue, setQueue] = React.useState(() => (Array.isArray(deck.items) ? deck.items : []))
      ...
      const items = queue
      const item = items.length === 0 || idx >= items.length ? null : items[idx] ?? null

      // 当前卡若关联过演示，拉一份场景来（画布随卡展示）；
      // 换卡先清掉旧图，避免新卡短暂渲染上一题的示意图
      React.useEffect(() => {
        if (item === null) return
        setScene(null)
        if (demoSceneCache.has(item.id)) { setScene(demoSceneCache.get(item.id) ?? null); return }
        let alive = true
        apiGet(`/demos?itemId=${encodeURIComponent(item.id)}&full=true&limit=1`).then(
          (data) => {
            const d = data?.demos?.[0] ?? null
            const sc = d === null || d.scene === undefined ? null : d.scene
            demoSceneCache.set(item.id, sc)
            if (alive) setScene(sc)
          },
      ...
      const grade = React.useCallback((key) => {
        ...
        setRevealed(false)
        // 没记住的卡塞回本轮队尾当场再过一遍（每张仅一次）
        if (key === 'again' && !againRef.current.has(item.id)) {
          againRef.current.add(item.id)
          setQueue((q) => [...q, item])
        }
        setIdx((i) => i + 1)
        apiPost('/review', { grades: [{ id: item.id, grade: key, elapsedMs }] }).catch((e) => setError(String(e.message ?? e)))
      }, [item])
      ...
      if (items.length === 0) {
        return h('div', { className: 'hst_card hst_review' }, h('div', { className: 'hst_reviewEnd' }, [
          h('div', { key: 't', className: 'hst_empty' }, '今日复习队列是空的。\n可以让助手把刚讲完的题写成卡片，或去「题库」导入内置卡片包。'),
          h('span', { key: 'h', className: 'hst_hint' }, `到期 ${deck.counts?.due ?? 0} · 新卡预算 ${deck.counts?.newBudget ?? 0}`),
          onRestart === undefined ? null : h('button', { key: 'r', className: 'hst_btn hst_btnSm', type: 'button', onClick: onRestart }, '再取一轮'),
        ]))
      }

      if (item === null) {
        const done = tally.again + tally.hard + tally.good + tally.easy
        return h('div', { className: 'hst_card hst_review' }, [
          h('div', { key: 'd', className: 'hst_reviewEnd' }, [
            h('div', { key: 't', className: 'hst_empty' }, done > 0
              ? `本轮作答 ${done} 次 · 掌握 ${tally.good + tally.easy} · 吃力 ${tally.hard} · 需重来 ${tally.again}\n答错的题会在 20 分钟后重新进入队列`
              : '今日队列已清空，休息一下 🎉'),
            h('div', { key: 'a', className: 'hst_row', style: { justifyContent: 'center' } }, [
              onRestart === undefined ? null : h('button', { key: 'r', className: 'hst_btn', type: 'button', onClick: onRestart }, '再取一轮'),
            ]),
            h('p', { key: 'h', className: 'hst_hint' }, onRestart === undefined
              ? '跟助手说「再来一组」可以换一批；把刚讲完的题也录成卡片，明天就能在这里复习。'
              : '也可以在对话里说「抽查我」，翻卡同样发在对话里。'),
          ]),
        ])
      }

      const s = subjectOf(item.subject)
      // 超长题干适当收起（完整内容存在题库里，「题库」页可看）；答案不截断
      const fit = (text) => (typeof text === 'string' && text.length > 900 ? `${text.slice(0, 900)}…（更完整的题干可在「题库」页查看）` : text)
      return h('div', { className: 'hst_card hst_review' }, [
        h('div', { key: 'top', className: 'hst_reviewTop' }, [
          ...
          h('div', { key: 'r', className: 'hst_row' }, [
            ...
            h('span', { key: 'kb', className: 'hst_hint' }, '空格翻面 · 1-4 评分'),
        ...
        error === null ? null : h('p', { key: 'e', className: 'hst_err' }, `评分请求失败：${error}`),
        // 双栏：有演示图时题目（含翻面后的答案）在左、图示在右，一屏放下不用滚
        h('div', { key: 'grid', className: `hst_reviewGrid${scene === null ? '' : ' hst_reviewGrid2'}` }, [
          h('div', { key: 'col', className: 'hst_reviewCol' }, [
            h('div', { key: 'q', className: 'hst_q' }, fit(item.question)),
            revealed ? h(AnswerBlock, { key: 'a', item }) : null,
          ]),
          scene === null ? null : h('div', { key: 'frame', className: 'hst_reviewFrame' }, h(DemoFrame, {
            key: 'f',
            scene,
            token: `deck-${item.id}`,
            // 抽题图示区（纯画布+步骤条）：
            //   · 翻面前 questionOnly —— 只显示带 q 标记的题面信息（隐藏步骤条）
            //   · 翻面后放开 —— 完整步骤时间轴可交互，图示进入详细讲解
            mode: 'panel',
            minHeight: 280,
            cap: 460,
            compact: true,
            ...
            ratio: revealed ? 0.6 : 0.78,
          })),
        ]),
        revealed
          ? h('div', { key: 'g', className: 'hst_reviewFoot' }, [
            h(GradeButtons, { key: 'gb', item, onGrade: grade }),
    ...
    /**
     * 设置页小卡片抽题入口：取今日队列 → 交给 DeckCard 翻卡——
     * 显示题目 → 心里/纸上作答 → 显示答案 → 四档评分写入排期。
     * 与对话里的 tutor_review_deck 翻卡共用同一组件，只是数据来源不同。
     * @param {{subject?: string, limit?: number, onExit?: () => void}} props。
     * @returns {object} React 元素。
     */
    function QueueRunner({ subject, limit, onExit }) {
      ...
      const load = React.useCallback(() => {
        setError(null)
        setDeck(null)
        apiGet(`/queue?limit=${Number.isFinite(limit) ? limit : 30}${subject ? `&subject=${subject}` : ''}`).then(
          (data) => setDeck({ items: Array.isArray(data.items) ? data.items : [], counts: data.counts ?? {} }),
      ...
      // 弹层形态：Esc / 点背景关闭（设置面板与徽标浮层都太窄，横版卡片放不下）
      React.useEffect(() => {
        if (onExit === undefined) return undefined
        const onKey = (event) => { if (event.key === 'Escape') onExit() }
        window.addEventListener('keydown', onKey)
        return () => window.removeEventListener('keydown', onKey)
      }, [onExit])
      const inModal = (content) => onExit === undefined
        ? content
        : h('div', {
          key: 'qz',
          className: 'hst_quizWrap',
          onMouseDown: (event) => { if (event.target === event.currentTarget) onExit() },
        }, h('div', { className: 'hst_quiz' }, content))

      if (error !== null) {
        return inModal(h('div', { className: 'hst_card hst_review' }, [
          h('p', { key: 'e', className: 'hst_err' }, `复习队列加载失败：${error}`),
      ...
      if (deck === null) return inModal(h('div', { className: 'hst_card hst_review' }, h('p', { className: 'hst_hint' }, '正在取今日队列…')))
      return inModal(h(DeckCard, {
        key: `${tick}-${subject ?? 'all'}`,
        deck,
        onRestart: () => { setTick((t) => t + 1); load() },
        onExit,
      }))
    }
    ...
    // ── 今日 ────────────────────────────────────────────────────────────────
    /**
     * 今日标签页：倒计时、目标进度、待复习分布、小卡片抽题入口、快速记时长、薄弱点。
    ...
    function TodayTab({ overview, onGoLibrary }) {
      ...
      if (running !== null) {
        // 宽屏弹层：窄设置面板放不下横版卡片，复习在居中大卡片里进行
        return h(QueueRunner, { key: 'r', subject: running.subject, onExit: () => setRunning(null) })
      }
      ...
      return h('div', { className: 'hst_body' }, [
        ...
        // 开始复习：小卡片抽题（题目 → 心里/纸上作答 → 显示答案 → 四档评分）
        h('div', { key: 'go', className: 'hst_card' }, [
          ...
          h('p', { key: 'p', className: 'hst_hint' }, '也可以在对话里说「抽查我」，翻卡同样发在对话里。'),
        ]),
        // 快速记时长
     ...
     * @returns {object} React 元素。
     */
    function DemoFrame({ scene, token, mode, onStep, minHeight, cap: capProp, bare: bareProp, ratio: ratioProp, compact: compactProp, lockFirst: lockFirstProp, questionsOnly: questionsOnlyProp }) {
      ...
      const send = React.useCallback(() => {
        ...
        win.postMessage({
          ...
          lockFirst: lockFirstProp === true,
          questionsOnly: questionsOnlyProp === true,
          ratio: Number.isFinite(ratioProp) ? ratioProp : undefined,
        }, '*')
      }, [scene, token, mode, themeTick, bareProp, compactProp, lockFirstProp, questionsOnlyProp, ratioProp])
      ...
      React.useEffect(() => {
        const onMessage = (ev) => {
          ...
          if (data.type === 'hst:height' && Number.isFinite(data.height)) {
            // 卡片态已是固定 16:9（对话讲题卡）；面板/侧栏/抽题图示区按内容自适应，
            // cap 兜底：图示再长也不超过这个高度，多余部分由引擎缩放进画布
            if (mode === 'panel') {
              const cap = Number.isFinite(capProp) ? capProp : 2000
              setHeight(Math.min(cap, Math.max(140, Math.ceil(data.height))))
            }
    ...
    /**
     * tutor_review_deck 的工具视图：复习翻卡套组。
     * 独立组件（不经过 DemoToolView 的演示 hook 链）——工具调用从「运行中」
     * 到「落定」会经历两次渲染，共用组件会在 hook 数量上不一致而崩溃。
     * @param {{block: object}} props DSH 传入的 { callId, toolName, block }。
     * @returns {object} React 元素。
     */
    function DeckToolView({ block }) {
      const meta = block !== null && typeof block === 'object'
        && block.meta !== null && typeof block.meta === 'object'
        ? block.meta
        : null
      if (meta === null || meta.kind !== 'hst-deck') {
        return h('div', { className: 'hst_toolLine' }, meta === null ? '复习翻卡 · 生成中…' : `复习翻卡 · ${meta.items?.length ?? 0} 张`)
      }
      return h(DeckCard, { deck: meta })
    }
    ...
    function DemoToolView({ callId, block }) {
      ...
      // 演示已在别处显示时卡片让位：不再渲染第二个 iframe，只留标题、摘要、
      // 题目答案与重点步骤——讲题卡本体（题目→答案→评分）始终完整展示。
      ...
      // 讲题卡：题目 + 答案 + 解析（meta 由 tutor_visualize 投影，带 item 时才有）
      const hasQuestion = meta.question !== null && meta.question !== undefined && meta.question !== ''
      const hasAnswer = meta.answer !== null && meta.answer !== undefined && meta.answer !== ''
      const qaItem = { answer: meta.answer ?? '', explanation: meta.explanation ?? '', source: meta.source ?? '', srs: null, preview: meta.itemPreview, masteryScore: NaN }
      const doGrade = React.useCallback((key) => {
        if (meta === null || meta.itemId === null || meta.itemId === undefined) return
        setGraded(key)
        apiPost('/review', { grades: [{ id: meta.itemId, grade: key, elapsedMs: 0 }] })
          .catch(() => setGraded(null))
      }, [meta])
      const gradedLabel = graded === null ? '' : (GRADE_BUTTONS.find((g) => g.key === graded)?.label ?? graded)
      ...
      return h('div', { className: `hst_toolCard${elsewhere ? ' hst_toolCardDocked' : ''}` }, [
        ...
        // 讲题卡 · 题目区（有 item 才显示；演示推到侧栏时题目答案仍完整）
        hasQuestion ? h('div', { key: 'qa', className: 'hst_qa' }, [
          h('div', { key: 'h', className: 'hst_row' }, [
            meta.subject === null ? null : h(Chip, { key: 'c', subject: meta.subject, active: true }),
            meta.topic === '' ? null : h('span', { key: 't', style: { fontSize: 13, fontWeight: 600 } }, meta.topic),
            meta.difficulty === null || meta.difficulty === undefined ? null : h('span', { key: 'd', className: 'hst_tag' }, `难度 ${meta.difficulty}`),
          ]),
          h('div', { key: 'q', className: 'hst_q' }, meta.question),
        ]) : null,
        // 只有哪儿都没显示时才自己画
        // 对话讲题卡：16:9 画布干净呈现（隐藏帧内标题/步骤说明/步骤条——
        // 卡片自带重点步骤芯片可驱动跳步），ratio 与 16:9 画幅一致，内容不被裁切
        elsewhere ? null : h(DemoFrame, {
          key: 'f',
          scene: meta.scene,
          token,
          mode: 'card',
        ...
        // 讲题卡 · 答案区：翻面对照 + 四档评分（直接写入复习排期）
        !hasAnswer ? null : revealed
          ? [
            h(AnswerBlock, { key: 'ans', item: qaItem }),
            graded === null
              ? h('div', { key: 'grade', className: 'hst_reviewFoot' }, [
                h(GradeButtons, { key: 'gb', item: qaItem, onGrade: doGrade }),
                h('p', { key: 'kb', className: 'hst_hint' }, '评分直接写入艾宾浩斯复习排期。'),
              ])
              : h('p', { key: 'ok', className: 'hst_ok' }, `已记录「${gradedLabel}」：${fmtInterval(qaItem.preview?.[graded] ?? 0)} 后复习这题。`),
          ]
          : h('div', { key: 'reveal', className: 'hst_row' }, [
            h('button', { key: 'b', className: 'hst_btn hst_btnPrimary', type: 'button', onClick: () => setRevealed(true) }, '显示答案'),
            h('span', { key: 'h', className: 'hst_hint' }, '先自己作答，再翻面对照'),
          ]),
        h(KeyStepChips, {
          key: 'k',
          keySteps: meta.keySteps,
    ...
    /**
     * 会话标题栏徽标：待复习条数 + 高考倒计时，点开看今日分布、可直接小卡片快速复习。
     * @returns {object|null} React 元素。
     */
    function TutorBadge() {
      const overview = useResource('/overview')
      ...
      return h('div', { className: 'hst_popWrap', ref: wrapRef }, [
        ...
        open ? h('div', { key: 'pop', className: 'hst_pop' }, [
          ...
          quick
            ? h(QueueRunner, { key: 'run', limit: 12, onExit: () => setQuick(false) })
            : h('div', { key: 'act', className: 'hst_row' }, [
              h('button', {
                key: 'r',
                className: 'hst_btn hst_btnSm hst_btnPrimary',
              ...
              h('span', { key: 'h', className: 'hst_hint' }, '也可在对话里说「抽查我」，完整面板在 设置 → 高中助学'),
            ]),
        ]) : null,
      ])
    }
    ...
    function apply(ctx) {
      ...
      // tutor_visualize / tutor_review_deck 的专属工具视图：按工具名 keyed，属于
      // 加性注册（不会顶掉宿主的通用工具行，也不影响其它工具的渲染）。
      // 前者是讲题卡（题目 → 16:9 演示 → 答案 → 四档评分），后者是复习翻卡套组。
      ...
      ctx.slots.inject('tool.call.toolview', () => ctx.slots.register({
        name: 'tool.call.toolview',
        id: 'highschool-tutor-deck',
        key: 'tutor_review_deck',
      }, DeckToolView))
    ...
    exports.DeckToolView = DeckToolView
