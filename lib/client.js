...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      '.hst_quizWrap{position:fixed;inset:0;z-index:130;background:#0b0e1473;display:flex;align-items:center;justify-content:center;padding:28px 20px;animation:hst_fade .16s ease}',
      '.hst_quiz{width:min(920px,94vw);max-height:88vh;overflow:auto;background:var(--dsw-alias-bg-layer-3,#fff);border:1px solid var(--dsw-alias-border-l2,#0000001a);border-radius:16px;box-shadow:0 24px 64px #00000045;padding:18px 20px 22px;display:flex;flex-direction:column;gap:12px}',
      '@keyframes hst_fade{from{opacity:0}to{opacity:1}}',
      '@media (prefers-reduced-motion: reduce){.hst_quizWrap{animation:none}}',
      // 弹层内的阅读列宽：横版弹层里正文不拉成超长行
      '.hst_quiz .hst_body{width:100%}',
      '.hst_quiz .hst_body > *{width:100%;max-width:780px;margin-inline:auto}',
      // 图表
     ...
     * @returns {object} React 元素。
     */
    function DeckCard({ deck, onRestart }) {
    ...
    function QueueRunner({ subject, limit, onExit }) {
      ...
      return inModal(h('div', { className: 'hst_body' }, [
        h('div', { key: 'h', className: 'hst_row', style: { justifyContent: 'space-between' } }, [
          h('span', { key: 't', className: 'hst_h3' }, subject === undefined ? '全科复习' : `${subjectOf(subject).label}复习`),
          h('span', { key: 'k', className: 'hst_hint' }, '空格翻面 · 1-4 评分'),
          onExit === undefined ? null : h('button', { key: 'x', className: 'hst_btn hst_btnSm', type: 'button', onClick: onExit }, '退出'),
        ]),
        h(DeckCard, { key: `${tick}-${subject ?? 'all'}`, deck, onRestart: () => { setTick((t) => t + 1); load() } }),
      ]))
    }
