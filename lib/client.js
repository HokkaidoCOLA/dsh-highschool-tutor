 ...
 *        今日   倒计时、今日待复习、复习闯关（翻卡+四档评分）、快速记时长、薄弱点
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      // 复习闯关
      '.hst_review{display:flex;flex-direction:column;gap:12px;min-height:230px}',
    ...
    function ReviewRunner({ subject, compact, onExit }) {
      ...
      if (queue === null) return h('div', { className: 'hst_card hst_review' }, h('p', { className: 'hst_hint' }, '正在取今日队列…'))

      if (item === null) {
        const done = tally.again + tally.hard + tally.good + tally.easy
        return h('div', { className: 'hst_card hst_review' }, [
          h('div', { key: 'd', className: 'hst_empty' }, done > 0
